...
TEST(UtilsMisc, LoadSeedDetails) {
    ...
    expect_error([&]() { chihaya::internal_misc::load_seed_details(H5::H5File(path, H5F_ACC_RDONLY), "seed", ritsuko::Version(1, 1, 0), clbk); }, "failed to validate");

    {
        H5::H5File fhandle(path, H5F_ACC_TRUNC);
        mock_array_opener(fhandle, "seed", { 13, 14 }, 1000000, "INTEGER");
    }
    auto deets = chihaya::internal_misc::load_seed_details(H5::H5File(path, H5F_ACC_RDONLY), "seed", ritsuko::Version(1, 0, 0), clbk); 
